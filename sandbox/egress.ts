 ...
 */
export function renderNftRules(
  ips: string[],
  dnsIp: string = SLIRP_DNS,
  proxy?: { address: string; port: number },
): string {
  ...
  if (proxy) {
    // The one destination the box is *meant* to reach: the proxy, across its own link. Without this
    // the policy drops the box's own SYN to the proxy — the link's address is not in any allowlist and
    // the port is not 80/443 — and the failure is silent: measured, the turn hung for its whole
    // timeout with no error and nothing in the proxy's log, because a dropped packet looks like a
    // slow network rather than a refusal.
    lines.push(`    ip daddr ${proxy.address} tcp dport ${proxy.port} accept`)
  }
...
export async function ensureEgressPolicy(
  ...
  return { rules: renderNftRules(addresses, SLIRP_DNS, proxy), unresolved }
}
...
export async function runtimeForEgress(
  egress: EgressMode,
  opts: { rootfs?: string; allowHosts?: string[]; backend?: BackendId; proxy?: { address: string; port: number } } = {},
  ...
  if (opts.backend === "container") {
    ...
    const policy = await ensureEgressPolicy(opts.allowHosts ?? [], opts.proxy)
