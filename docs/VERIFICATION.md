...
reasoning and no annotation of the model's intent. It *does* now run the project's own checks
against exactly the accepted subset before anything is written (SPEC §4.1), so "take hunk 2 and
reject the hunk that makes it compile" is caught rather than written. What that is not is a
guarantee the subset is coherent in any deeper sense: it re-runs the check the project already
declares, so a project with no detectable check, or a subset wrong in a way those checks do not
cover, still goes through. The check is a floor, not a proof, and the decision remains the user's.
