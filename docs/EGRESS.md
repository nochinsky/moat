...
* **The doctor measures the box the agent gets, proxy included — and getting there cost three wrong
  diagnoses of one missing newline.** `runIsolationChecks` used to drop the `egressProxy` its caller
  spreads in beside `egress`/`egressRules`, so with `--egress-proxy` the doctor probed a box *without*
  the proxy: a different, more permissive box than the agent gets, which is the `injectedVarNames`
  defect class. It also could not simply be handed the proxy, because its reachability probe was
  `$MOAT_PROBE_TIMEOUT /bin/bash -c 'exec 3<>/dev/tcp/<provider>/<port>'` — and `/dev/tcp/<host>/<port>`
  resolves the host *in the box*, where a proxied box keeps no resolver. So the probe goes through the
  proxy now: `CONNECT` over `/dev/tcp` to the proxy's own address, read from its status line, where
  `200` means the proxy dialed it and a refusal means the policy did. Measured, with the control:

  ```
  proxied  egress filtered  … api.deepseek.com:443 is reachable through the proxy   pass
  control  egress filtered  … api.deepseek.com:443 is reachable                     pass (unchanged)
  ```

  The reason this took three attempts is worth keeping: the generated probe joined its statements with
  a **literal backslash-n** instead of a newline, so the shell read `\ncase` as a *command name*, the
  `case` never ran, and the variable the row reads was never emitted. The row then reported a working
  proxy as unreachable — and because the same commands typed into the box by hand worked (real
  newlines), the reading looked like a readiness problem. It was "fixed" once for that reason and
  reverted when it did not help, then "explained" as a first-connection effect after `CONNECT` worked
  where an absolute-URI `GET` had not. Both were the same missing newline: with two probes, the *first*
  one's `echo` was glued to its assignment and the second one's `case` survived, so the symptom moved
  with the probe order and looked like a timing asymmetry in both directions.

  Two instruments ended it, and they are the ones to reach for first next time. The script moat actually
  ran is on disk — `grep -rl MOAT_ALLOWED_REACHABLE ~/.moat/envs/<id>` finds it under
  `rootfs/.moat/entry-*.sh`, and the broken line is visibly broken in it. And the generator is exported
  with a test over its output, `test/unit/proxy-doctor.test.ts`, which asserts the `case` begins its own
