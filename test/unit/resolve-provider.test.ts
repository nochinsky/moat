...
test("--base-url overrides what the environment recorded", () => {
  const state = { provider: CUSTOM_ENDPOINT.id, providerBaseUrl: "http://old/v1", model: "moat/old" }
...
test("no state and no flags is DeepSeek and its default model", () => {
  ...
  assert.equal(provider.id, DEEPSEEK.id)
